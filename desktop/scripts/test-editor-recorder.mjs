...
try {
  ...
  caseId = await page.evaluate(async () => {
    const response = await fetch('http://127.0.0.1:5001/api/ui-automation/cases', {
      ...
      body: JSON.stringify({ name: `录制工作台验收-${Date.now()}` }),
    })
    const body = await response.json()
    if (!response.ok || !body?.id) throw new Error(`RECORDER_FIXTURE_CREATE_FAILED:${response.status}:${JSON.stringify(body)}`)
    return body.id
  })
  ...
  if (!text.includes('用户操作流程') || !text.includes('搜索并添加') || !text.includes('浏览器控制台') || !text.includes('网络请求')) throw new Error('Studio layout is incomplete')
