  ...
  <div class="automation-studio" v-loading="loading">
    ...
    <div class="studio-workspace">
      <main class="flow-canvas">
        ...
        <div v-if="!steps.length" class="empty-flow"><el-icon><VideoCamera /></el-icon><h3>录制真实操作，快速生成用例</h3><p>输入地址后点击“开始录制”，在打开的浏览器中完成登录、搜索、上传或下单。也可以手工添加步骤。</p><div><el-button type="danger" :icon="VideoCamera" :disabled="!desktopReady || recordingStarting" :loading="recordingStarting" @click="startRecording">开始录制</el-button><el-button @click="openPicker(0)">手工添加</el-button></div></div>
...
const recording=ref(false),recordingStarting=ref(false),authCaptureMode=ref(false),refreshAuthStateId=ref(null),recorderMode=ref('record'),recorderUrl=ref(''),consoleLogs=ref([]),networkLogs=ref([]),executionLogs=ref([]),variableSnapshot=ref({}),debugTab=ref('execution')
...
async function clearSteps(){try{await ElMessageBox.confirm('确认清空全部步骤？','清空流程',{type:'warning'});steps.value=[];selectedStepId.value=''}catch(error){if(error!=='cancel'&&error!=='close')ElMessage.error('清空步骤失败：'+friendlyError(error))}}
...
function currentAuthValidationUrl(){const first=steps.value.find(step=>step.enabled!==false&&step.type==='goto')?.input?.url||'';if(/^https?:\/\//i.test(first))return first;if(caseData.value?.base_url&&first){try{return new URL(first,caseData.value.base_url).toString()}catch(error){console.warn('无法拼接登录态校验地址',error)}}return /^https?:\/\//i.test(caseData.value?.base_url||'')?caseData.value.base_url:''}
...
async function cancelAuthCapture(){
  try{await window.testmaster.recorder.stop()}catch(error){console.warn('停止登录态录制失败',error)}
...
async function startRecording(){
  ...
  try{
    ...
    try{await window.testmaster.recorder.start({url,slowMo:50,authStateId:selectedAuthStateId.value},handleRecorderEvent);recording.value=true;recorderMode.value='record';ElMessage.success('录制已开始，请在打开的浏览器中操作')}catch(error){if(activeCaptureId.value)await autoTestRequest.post(`/auto-test/import/captures/${activeCaptureId.value}/cancel`,{reason:'browser recorder failed to start'}).catch(()=>{});activeCaptureId.value=null;ElMessage.error(`录制启动失败：${friendlyError(error)}`)}
  }finally{recordingStarting.value=false}
}
function queueCaptureExchange(event){
  if(!activeCaptureId.value||capturePaused.value||!['xhr','fetch','document'].includes(event.resourceType))return
  captureQueue.push({captureEventId:event.captureEventId,method:event.method,url:event.url,status:event.status,resourceType:event.resourceType,requestHeaders:event.requestHeaders,requestBody:event.requestBody,responseHeaders:event.responseHeaders,responseBody:event.responseBody,timingMs:event.durationMs,pageUrl:event.pageUrl,failureReason:event.failureReason});capturePendingCount.value=captureQueue.length
...
<style scoped>
.automation-studio{display:flex;flex-direction:column;gap:12px;min-width:900px;color:var(--tm-text-primary)}.studio-toolbar,.case-heading,.case-heading>div,.toolbar-actions,.recording-bar,.case-context,.canvas-header,.canvas-header>div,.step-tools,.recording-indicator{display:flex;align-items:center}.studio-toolbar{justify-content:space-between;padding-bottom:12px;border-bottom:1px solid var(--border-subtle)}.case-heading{gap:12px}.case-heading>div{gap:12px}.case-heading h1{margin:0;font-size:20px}.engine-state{color:var(--tm-color-danger);font-size:12px}.engine-state.ready{color:var(--tm-color-success)}.engine-state i,.recording-indicator i{display:inline-block;width:7px;height:7px;border-radius:50%;background:currentColor;margin-right:5px}.toolbar-actions{gap:8px}.recording-bar{min-height:48px;gap:8px;padding:8px 12px;background:var(--tm-bg-card);border:1px solid var(--border-subtle);border-radius:6px}.recording-bar.active{border-color:var(--tm-color-danger);background:color-mix(in srgb, var(--tm-color-danger) 8%, transparent)}.record-address{flex:1}.recording-indicator{color:var(--tm-color-danger);font-weight:700;font-size:12px}.auth-capture-tip{font-size:12px;color:#5f6b64;flex:1}.auth-state-manager{display:flex;flex-direction:column;gap:8px;max-height:340px;overflow:auto}.auth-row{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;padding:8px;border-bottom:1px solid #edf0ee}.auth-row>div{display:flex;flex-direction:column;min-width:0}.auth-row small{color:#77827b;margin-top:2px}.auth-health{font-size:11px;margin-top:3px}.auth-health.ok{color:var(--tm-color-success)}.auth-health.bad{color:var(--tm-color-danger)}.auth-empty{padding:18px;text-align:center;color:#77827b}.recording-indicator i{animation:pulse 1.2s infinite}.case-context{gap:10px;padding:10px 14px;background:var(--tm-bg-card);border:1px solid var(--border-subtle);border-radius:6px}.case-context label{font-size:12px;font-weight:700}.case-context .el-input{max-width:640px}.studio-workspace{display:flex;min-height:570px;background:var(--tm-bg-card);border:1px solid var(--border-subtle);border-radius:6px;overflow:hidden}.flow-canvas{flex:1;min-width:0;padding:18px 22px;background:#f8faf9}.canvas-header{justify-content:space-between;margin-bottom:18px}.canvas-header h2{margin:0;font-size:17px}.canvas-header span{color:#748078;font-size:12px}.canvas-header>div{gap:8px}.empty-flow{min-height:430px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center}.empty-flow>.el-icon{font-size:38px;color:var(--tm-color-danger)}.empty-flow h3{margin:14px 0 6px}.empty-flow p{max-width:540px;margin:0 0 18px;color:#69756e;line-height:1.7}.flow-list{max-width:900px;margin:0 auto}.flow-step{display:grid;grid-template-columns:34px 34px minmax(0,1fr) auto auto;gap:10px;align-items:center;padding:11px 12px;background:#fff;border:1px solid #d8e1db;border-radius:6px;cursor:pointer}.flow-step:hover{border-color:#8bb39e}.flow-step.selected{border-color:var(--tm-color-success);box-shadow:0 0 0 2px rgba(40,114,79,.1)}.flow-step.invalid{border-color:#d33d43}.flow-step.disabled{opacity:.55}.step-sequence{width:28px;height:28px;display:grid;place-items:center;border-radius:50%;background:#e8f3ec;color:var(--tm-color-success);font-weight:700}.step-icon{color:var(--tm-color-success)}.step-summary{min-width:0;display:flex;flex-direction:column}.step-summary strong{font-size:13px}.step-summary span{overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#6e7a72;font-size:11px;margin-top:3px}.step-summary small{color:var(--tm-color-danger);margin-top:3px}.step-badges{display:flex;gap:4px}.step-tools{gap:0}.insert-step{width:100%;height:24px;border:0;background:transparent;display:flex;align-items:center;justify-content:center;gap:5px;color:#859188;font-size:11px;cursor:pointer;opacity:.25}.insert-step:hover,.insert-step.first{opacity:1;color:var(--tm-color-success)}.insert-step.first{margin-bottom:5px}.debug-console{height:210px;background:var(--tm-bg-card);border:1px solid var(--border-subtle);border-radius:6px;padding:0 14px;overflow:hidden}.debug-list{height:140px;overflow:auto;font-family:Consolas,monospace;font-size:11px}.debug-line{display:grid;grid-template-columns:80px 1fr;gap:8px;padding:5px;border-bottom:1px solid #eef1ef}.debug-line span{color:#879189}.debug-line.error strong{color:var(--tm-color-danger)}@keyframes pulse{50%{opacity:.25}}@media(max-width:1150px){.inspector{width:300px}.recording-bar{flex-wrap:wrap}.record-address{flex-basis:100%}}
</style>
