<template>
    ...
    <template v-if="selectedNode.type === 'ThreadGroup'">
      ...
      <div class="form-section">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ...
          <el-button size="small" @click="$emit('add-child-to-current', 'AggregateReport')">📊 聚合报告(高级)</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'ResponseTimeGraph')">📉 响应时间图</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'CSVDataSet')">📄 CSV 数据源</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'JDBCSampler')">🗄️ JDBC 请求</el-button>
        </div>
      </div>
      <div class="form-section">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ...
          <el-button size="small" @click="$emit('add-child-to-current', 'TransactionController')">📦 事务控制器</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'ThroughputController')">⏱️ 吞吐量控制器</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'OnceOnlyController')">1️⃣ 仅一次控制器</el-button>
        </div>
      </div>
      <div class="form-section">
    ...
    <template v-if="selectedNode.type === 'HttpSampler'">
      ...
      <div class="form-section">
        ...
        <div style="display:flex;gap:4px;flex-wrap:wrap">
          ...
          <el-button size="small" @click="$emit('add-child-to-current', 'BeanShellAssertion')">💻 BeanShell</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'JSR223Assertion')">🔥 JSR223</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'SizeAssertion')">📏 大小</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'XPathAssertion')">🗂️ XPath</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'CompareAssertion')">⚖️ 比较</el-button>
          <el-button size="small" @click="$emit('add-child-to-current', 'XMLAssertion')">📜 XML</el-button>
        </div>
      </div>
      <div class="form-section">
      ...
      <div class="form-section">
        ...
        <div style="display:flex;gap:6px;flex-wrap:wrap">
          ...
          <el-button size="small" @click="$emit('add-child-to-current', 'JSR223PreProcessor')">🔥 JSR223前置</el-button>
        </div>
      </div>
      <div class="form-section">
      ...
      <div class="form-section" v-if="projectVariables.length > 0">
        <div class="vars-panel">
          ...
          <div class="vars-chips">
            <span v-for="v in projectVariables" :key="v.name" class="var-chip" @click="$emit('insert-variable', v.name)" :title="v.source">{{ v.name }}</span>
          </div>
        </div>
      </div>
          ...
          <div class="vrt-left-col">
            ...
            <div class="vrt-sample-list">
              <div v-for="(s, si) in filteredSamples" :key="si"
                class="vrt-sample-item" :class="{ active: selectedSampleIdx === si }" @click="$emit('select-sample', si)">
  ...
  <div class="editor-empty" v-else>
    ...
    <div class="editor-empty-tips">
      <div class="empty-tip" @click="$emit('add-root-element')">➕ 添加线程组</div>
      <div class="empty-tip" @click="$emit('go-to-step1')">📥 导入接口用例</div>
    </div>
  </div>
</template>
