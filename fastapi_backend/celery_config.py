...
# 若未配置 Redis，则降级为内存模式（仅本地开发/测试可用，不用于生产）
if not broker_url:
    broker_url = 'memory://'
    _logger.warning("CELERY_BROKER_URL 未配置，Celery 降级为内存模式（仅本地开发/测试）")
if not result_backend:
    result_backend = 'cache+memory://'
    _logger.warning("CELERY_RESULT_BACKEND 未配置，Celery 结果后端降级为内存模式（仅本地开发/测试）")
