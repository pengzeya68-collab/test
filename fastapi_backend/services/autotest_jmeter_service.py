...
def export_cases_to_jmx(
    ...
    _JMX_COMMENT = "<!-- Generated by TestMaster - Compatible with JMeter 5.0+ (5.1.1/5.2/5.3/5.4/5.5/5.6) -->"
    # XML 规范要求声明必须在第一行，注释插在 XML 声明之后
...
def _parse_hash_tree_children(hash_tree):
    ...
    while i < len(siblings):
        ...
        if tag in _IGNORED_TYPES or tag.startswith(
            ("stringProp", "boolProp", "longProp", "intProp", "collectionProp", "elementProp", "value")
        ):
            i += 1
            continue
...
def _parse_assertion_props(elem, assertion_type):
    if assertion_type == "ResponseAssertion":
        ...
        patterns = []
        coll = elem.find(".//collectionProp[@name='Assertion.test_strings']")
        if coll is not None:
...
def _parse_controller_props(elem, ctrl_type):
    if ctrl_type == "IfController":
        ...
        percent = _get_float_prop(elem, "ThroughputController.percentThroughput", 100.0)
            ...
            for header_manager in sampler_hash_tree.findall("./HeaderManager"):
                for header_elem in header_manager.findall(".//elementProp[@elementType='Header']"):
                    ...
                    if name_elem is not None and value_elem is not None:
                        headers[name_elem.text or ""] = value_elem.text or ""
