...
def export_cases_to_jmx(
    ...
    jmeter_test_plan.set("jmeter", "5.0")
    ...
    _add_element_prop(
        ...
        str(thread_group_config.get("num_threads", 1)),
    )
    _add_element_prop(
        thread_group,
        "ThreadGroup.ramp_time",
        str(thread_group_config.get("ramp_time", 1)),
    )
    ...
    _add_element_prop(
        ...
        str(thread_group_config.get("loop_count", 1)),
    )
    ...
    _JMX_COMMENT = "\n<!-- Generated by TestMaster - Compatible with JMeter 5.0+ (5.1.1/5.2/5.3/5.4/5.5/5.6) -->"
    return formatted_xml.replace("<?xml", _JMX_COMMENT + "\n<?xml", 1)
...
def _parse_hash_tree_children(hash_tree):
    ...
    while i < len(siblings):
        ...
        if elem.tag == "hashTree":
            ...
            continue
        tag = elem.tag
        if tag in _IGNORED_TYPES or tag.startswith(
            (
                "stringProp",
                "boolProp",
                "longProp",
...
def _parse_sampler_props(elem):
    ...
    return {
        ...
        "body": body,
        "body_type": "json"
        if len(body) > 0 and (body.strip().startswith("{") or body.strip().startswith("["))
        else "form-data"
...
def _parse_controller_props(elem, ctrl_type):
    if ctrl_type == "IfController":
        ...
        percent = (
            _get_float_prop(elem, "ThroughputController.percentThroughput", 100.0)
            if hasattr(_get_float_prop, "__call__")
            else 100.0
        )
...
def _iter_http_samplers(parent):
    ...
    for index, child in enumerate(children):
        if child.tag in ("HTTPSamplerProxy", "HTTPSampler"):
            sampler_hash_tree = (
                children[index + 1] if index + 1 < len(children) and children[index + 1].tag == "hashTree" else None
            )
...
def export_tree_to_jmx(tree: List[Dict], plan_name: str = "Test Plan", plan_vars: List = None) -> str:
    ...
    _JMX_COMMENT = "\n<!-- Generated by TestMaster - Compatible with JMeter 5.0+ (5.1.1/5.2/5.3/5.4/5.5/5.6) -->"
    return result.replace("<?xml", _JMX_COMMENT + "\n<?xml", 1)
...
def _build_assertion(parent, name, props):
    ...
    _add_element_prop(
        a,
        "Assertion.test_field",
        "Assertion.response_code" if at == "status_code" else "Assertion.response_data",
    )
    _add_element_prop(a, "Assertion.assume_success", "false")
    _add_element_prop(
        a,
        "Assertion.test_type",
        "2" if at == "contains" else "8" if at == "matches" else "8",
    )
...
def _build_cookie_manager(parent, name, props):
    ...
    _add_element_prop(
        ...
        "true" if props.get("clearEachIteration") else "false",
    )
    ET.SubElement(parent, "hashTree")
...
def _build_loop_controller(parent, name, props, children):
    ...
    _add_element_prop(
        ...
        "true" if props.get("forever") else "false",
    )
    sh = ET.SubElement(parent, "hashTree")
    for child in children:
        _build_tree_node(sh, child)
...
def _build_transaction_controller(parent, name, props, children):
    ...
    _add_element_prop(
        ...
        "true" if props.get("includeTimers") else "false",
    )
...
def _build_throughput_controller(parent, name, props, children):
    ...
    _add_element_prop(
        ...
        "true" if props.get("perThread") else "false",
    )
    sh = ET.SubElement(parent, "hashTree")
    for child in children:
        _build_tree_node(sh, child)
...
def _build_beanshell_assertion(parent, name, props):
    ...
    _add_element_prop(
        ...
        "true" if props.get("resetInterpreter") else "false",
    )
    ET.SubElement(parent, "hashTree")
...
def _build_compare_assertion(parent, name, props):
    ...
    _add_element_prop(
        a,
        "CompareAssertion.test_field",
        props.get("testField", "Assertion.response_data"),
    )
    _add_element_prop(
        a,
        "CompareAssertion.test_type",
        cmp.get(props.get("compareType", "contains"), "6"),
    )
    _add_element_prop(
        a,
        "CompareAssertion.compare_content",
        "true" if props.get("compareContent") else "false",
    )
    cp = ET.SubElement(a, "collectionProp")
...
def _build_influxdb_backend_listener(parent, name, props):
    ...
    influxdb_args = [
        (
            "influxdbUrl",
            props.get("influxdbUrl", "http://localhost:8086/write?db=jmeter"),
        ),
...
def _build_foreach_controller(parent, name, props, children):
    ...
    _add_element_prop(
        ...
        "true" if props.get("useSeparator") else "false",
    )
