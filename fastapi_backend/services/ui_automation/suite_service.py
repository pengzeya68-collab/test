...
from fastapi_backend.models.ui_automation import UICase, UICaseVersion, UISuite, UISuiteItem
...
async def get_suite(db: AsyncSession, user_id: int, suite_id: int, *, with_items: bool = True) -> UISuite:
    query = select(UISuite).where(UISuite.id == suite_id, UISuite.user_id == user_id).execution_options(populate_existing=True)
    if with_items:
...
async def list_suites(db: AsyncSession, user_id: int) -> list[UISuite]:
    result = await db.execute(
        select(UISuite).options(selectinload(UISuite.items))
        .where(UISuite.user_id == user_id).order_by(UISuite.updated_at.desc(), UISuite.id.desc())
    )
