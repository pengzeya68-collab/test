...
class PytestDataDrivenEngine:
    ...
    def execute(
        self, steps: List[Dict], data_matrix: Dict[str, Any], env_vars: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            overall_duration = int((time.time() - start_time) * 1000)
            return {"success": False, "error": str(e), "duration": overall_duration, "report_url": None}
    ...
    def _run_pytest(self, test_file: Path, data_file: Path) -> Dict[str, Any]:
        ...
        if len(result_files) == 0 and result.returncode != 0 and "No module named pytest" in result.stderr:
            ...
            if pytest_exe.exists():
                ...
                result = subprocess.run(cmd2, capture_output=True, text=True, cwd=str(self.temp_dir), timeout=300)
                result_files = list(self.allure_results_dir.glob("*.json"))
            ...
            if len(result_files) == 0:
                ...
                result = subprocess.run(cmd3, capture_output=True, text=True, cwd=str(self.temp_dir), timeout=300)
                result_files = list(self.allure_results_dir.glob("*.json"))
