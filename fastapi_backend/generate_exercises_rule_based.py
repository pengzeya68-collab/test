...
def extract_keywords(content: str, num_keywords: int = 5) -> List[str]:
    ...
    # 如果关键词不足，添加一些常见测试术语
    test_terms = [
        "软件测试", "测试用例", "缺陷管理", "测试计划", "测试报告",
        "单元测试", "集成测试", "系统测试", "验收测试", "回归测试",
        "黑盒测试", "白盒测试", "灰盒测试", "自动化测试", "手工测试",
        "功能测试", "性能测试", "安全测试", "兼容性测试", "易用性测试",
        "测试覆盖率", "测试策略", "测试环境", "测试数据", "测试工具"
    ]
...
def generate_multiple_choice(section_title: str, content: str, keyword: str, difficulty: str = "medium") -> Dict[str, Any]:
    """生成多选题"""
    templates = [
        {
...
def generate_true_false(section_title: str, content: str, keyword: str, difficulty: str = "easy") -> Dict[str, Any]:
    ...
    templates = [
        {
            "title": f"{keyword}是软件测试中的重要概念。",
            "answer": "true"
        },
        {
            "title": f"{keyword}只需要在软件发布前进行。",
            "answer": "false"
        },
        {
            "title": f"测试人员应该尽早参与{keyword}活动。",
            "answer": "true"
        },
        {
            "title": f"{keyword}可以完全自动化，不需要人工干预。",
            "answer": "false"
        }
    ]
...
def generate_code_question(section_title: str, content: str, keyword: str, difficulty: str = "hard") -> Dict[str, Any]:
    ...
    templates = [
        ...
        },
        {
            ...
            "solution": f"def correct_function():\n    # 修复后的代码\n    return True",
            "test_cases": json.dumps([
                {"input": "", "expected": "True"}
            ])
        }
...
def generate_exercises_for_path(path_id: int, path_title: str) -> int:
    ...
    for idx, (section_id, section_title, section_content) in enumerate(sections, 1):
        ...
        # 生成该章节的习题
        exercises = generate_exercises_for_section(
            section_title, section_content, exercises_per_section
        )
