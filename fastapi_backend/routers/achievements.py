...
async def check_and_unlock_achievements(user_id: int, db: AsyncSession):
    ...
    # 只查询未解锁的成就，减少数据量
    all_achievements_stmt = select(Achievement).where(Achievement.id.notin_(unlocked_ids) if unlocked_ids else True)
    ...
    exam_count_stmt = select(func.count()).select_from(ExamAttempt).where(ExamAttempt.user_id == user_id)
    ...
    perfect_stmt = (
        ...
        .join(
            Exam,
            and_(ExamAttempt.exam_id == Exam.id, ExamAttempt.score == Exam.total_score),
        )
    ...
    interview_count_stmt = select(func.count()).select_from(InterviewSession).where(InterviewSession.user_id == user_id)
    interview_count_result = await db.execute(interview_count_stmt)
    interview_count = interview_count_result.scalar_one()

    streak_stmt = select(func.max(DailyCheckin.streak_count)).where(DailyCheckin.user_id == user_id)
...
async def get_achievements(
    ...
    unlocked_stmt = select(UserAchievement).where(UserAchievement.user_id == current_user.id)
    unlocked_result = await db.execute(unlocked_stmt)
