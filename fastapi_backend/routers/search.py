...
from fastapi_backend.models.models import User, Exercise, Post, Exam, LearningPath
...
async def global_search(
    ...
    if not category or category == "exercises":
        r = await db.execute(
            select(Exercise).where(
                or_(Exercise.title.contains(q), Exercise.description.contains(q))
            ).limit(page_size if category else 5)
        )
        results["exercises"] = [{"id": e.id, "title": e.title, "difficulty": e.difficulty, "knowledge_point": e.knowledge_point} for e in r.scalars().all()]
    ...
    if not category or category == "posts":
        ...
        results["posts"] = [{"id": p.id, "title": p.title, "summary": p.summary, "category": p.category} for p in r.scalars().all()]

    if not category or category == "exams":
        r = await db.execute(
            select(Exam).where(Exam.title.contains(q)).limit(page_size if category else 5)
        )
    ...
    if not category or category == "paths":
        r = await db.execute(
            select(LearningPath).where(
                or_(LearningPath.title.contains(q), LearningPath.description.contains(q))
            ).limit(page_size if category else 5)
        )
