...
from fastapi_backend.models.models import User, Progress, Exercise, ProjectSubmission, ExamAttempt, Note
...
async def get_learning_summary(
    ...
    ex_count = await db.scalar(
        select(func.count(Progress.id)).where(Progress.user_id == current_user.id, Progress.completed == True)
    ) or 0
    ...
    exam_count = await db.scalar(
        select(func.count(ExamAttempt.id)).where(ExamAttempt.user_id == current_user.id, ExamAttempt.is_completed == True)
    ) or 0
