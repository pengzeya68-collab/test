...
def current_user():
    async def user():
        return SimpleNamespace(id=1, username='suite-user', is_active=True)
...
def create_versioned_case(client, name, suffix):
    ...
    saved = client.put(f"/api/ui-automation/cases/{case['id']}/steps", json={'steps': [
        {'id': f'suite-{suffix}-open', 'order': 10, 'type': 'goto', 'input': {'url': '/{{path}}'}},
        {'id': f'suite-{suffix}-assert', 'order': 20, 'type': 'assert_title', 'input': {'expected': '{{title}}'}},
    ]})
...
async def test_suite_crud_and_data_driven_execution_plan(client):
    ...
    created = client.post('/api/ui-automation/suites', json={
        'name': '核心回归套件', 'description': '登录到下单', 'stop_on_first_failure': True,
    })
    assert created.status_code == 200, created.text
    suite = created.json()

    items = client.put(f"/api/ui-automation/suites/{suite['id']}/items", json=[
        {
            'case_id': case_a['id'], 'pinned_version_id': version_a['id'], 'order': 10,
            'data_source': {'rows': [
    ...
    duplicate = client.put(f"/api/ui-automation/suites/{suite['id']}/items", json=[
