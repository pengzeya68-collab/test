...
class TestInterviewSession:
    ...
    def test_create_session_success(self, mock_get_db, mock_get_user):
        ...
        mock_question = InterviewQuestion(id=self.question_id, title="测试题目", is_published=True)
    ...
    def test_create_session_without_auth(self):
        """测试未认证用户创建会话失败"""
        response = self.client.post("/api/v1/interview/sessions", json={"question_id": self.question_id})
    ...
    def test_list_my_sessions(self, mock_get_db, mock_get_user):
        ...
        mock_session = InterviewSession(id=1, user_id=self.user_id, question_id=self.question_id, status="started")
        mock_question_title = "测试题目"
        mock_question_difficulty = "easy"

        mock_result = MagicMock()
        mock_result.all.return_value = [(mock_session, mock_question_title, mock_question_difficulty)]
        ...
        response = self.client.get("/api/v1/interview/sessions", headers=self.headers)
