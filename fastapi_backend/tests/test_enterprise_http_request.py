...
from fastapi_backend.services.autotest_request_config import (
    mask_request_config,
    merge_request_config,
    protect_request_config,
    reveal_request_config,
)
...
async def test_urlencoded_and_graphql_bodies(monkeypatch):
    ...
    try:
        ...
        await service.execute_http_request(
            ...
            "query User { user { id } }",
            body_type="graphql",
            request_config={"graphql_variables": {"id": 7}},
            http_client=client,
        )
    finally:
        await client.aclose()
...
async def test_static_cookies_overlay_session_cookie_without_persisting():
    ...
    try:
        await service.execute_http_request(
            ...
            {},
            None,
            request_config={"cookies": {"tenant": "x"}},
            http_client=client,
        )
...
async def test_dynamic_auth_variables_and_non_idempotent_retry_policy(monkeypatch):
    ...
    try:
        result = await service.execute_http_request(
            ...
            {},
            {"count": 0},
            variables={"token": 'runtime"token\\line'},
            request_config={
