...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DATETIME(), nullable=True),
        sa.Column(
            "extractors", sa.TEXT(), server_default=sa.text("'[]'"), nullable=True
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "is_active", sa.BOOLEAN(), server_default=sa.text("1"), nullable=True
        ),
        sa.ForeignKeyConstraint(
            ["environment_id"],
            ["interface_test_environments.id"],
        ),
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["users.id"],
    ...
    op.create_table(
        "interview_question_collections",
        sa.Column("id", sa.INTEGER(), nullable=False),
        sa.Column("user_id", sa.INTEGER(), nullable=False),
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "user_id", "question_id", name=op.f("unique_user_interview_question")
        ),
    )
    op.create_table(
        "audit_logs",
    ...
    with op.batch_alter_table("test_scenarios", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_test_scenarios_webhook_token"), ["webhook_token"], unique=1
        )

    with op.batch_alter_table("auto_test_groups", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_auto_test_groups_id"), ["id"], unique=False
        )
