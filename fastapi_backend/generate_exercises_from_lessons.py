...
from fastapi_backend.models.models import Base, Exercise, LearningPath, LessonSection
...
class ExerciseGenerator:
    ...
    async def generate_exercises_for_all_paths(self):
        ...
        result = await self.db.execute(
            select(LearningPath).order_by(LearningPath.id)
        )
    ...
    async def generate_exercises_for_path(self, path: LearningPath):
        ...
        result = await self.db.execute(
            ...
            .where(LessonSection.learning_path_id == path.id)
        ...
        for section in sections:
            ...
            # 生成该题的习题
            exercises = await self.generate_exercises_from_section(
                section, exercises_per_section
            )
        ...
        if len(path_exercises) < 100:
            ...
            extra_exercises = await self.supplement_exercises(
                path, sections, needed
            )
        ...
        await self.db.execute(
            ...
            .values(exercise_count=len(path_exercises))
        )
    ...
    async def generate_exercises_from_section(
        self, section: LessonSection, count: int
    ) -> List[Dict[str, Any]]:
        """根据单个课程章节生成习题"""
        # 构建Prompt
...
"""
    ...
    def parse_ai_response(
        self, response: str, section: LessonSection
    ) -> List[Dict[str, Any]]:
        """解析AI返回的习题数据"""
        try:
            # 提取JSON（可能包含在```json ```中）
