...
LESSON_CONTENT_4["性能测试 - JMeter与Locust"] = [
    {
        ...
        "sort_order": 1, "knowledge_point": "性能指标", "time_estimate": 30,
        "content": """## 性能测试核心指标体系
...
> **小结**：性能指标体系是性能测试的"通用语言"。建议先与产品和开发团队一起定义SLI（服务水平指标）和SLO（服务水平目标），使性能测试有明确的对标依据。"""
    },
    {
        ...
        "sort_order": 2, "knowledge_point": "JMeter基础", "time_estimate": 35,
...
> **小结**：口诀——GUI写脚本、CLI跑压测、看报告用HTML、大规模靠分布式。"""
    },
    {
        ...
        "sort_order": 3, "knowledge_point": "JMeter脚本开发", "time_estimate": 40,
...
> **小结**：脚本开发完成后，先用1个线程、1次循环在GUI模式下调试通过，确认关联正确、断言有效，再切换到CLI模式执行正式压测。"""
    },
    {
        ...
        "sort_order": 4, "knowledge_point": "JMeter分布式", "time_estimate": 35,
        "content": """## JMeter分布式压测
...
> **小结**：分布式压测关键要点是网络可达、时间同步、配置一致。同时监控Slave节点本身资源——如果Slave CPU接近100%，说明Slave本身成为瓶颈，需要增加Slave数量。"""
    },
    {
        ...
        "sort_order": 5, "knowledge_point": "Locust开发", "time_estimate": 35,
...
> **小结**：Locust推荐用于复杂业务逻辑编排、CI/CD深度集成、Python技术栈团队。对于非开发背景测试人员，JMeter的GUI更友好。"""
    },
    {
        ...
        "sort_order": 6, "knowledge_point": "性能监控", "time_estimate": 35,
        "content": """## 性能监控体系
...
> **小结**：当TPS不再增长而CPU未满时，问题可能在应用层（线程池/连接池/锁竞争）；当CPU满而TPS不达标时，需要代码优化或扩容。"""
    },
    {
        ...
        "sort_order": 7, "knowledge_point": "性能调优", "time_estimate": 35,
...
> **小结**：核心原则——测量驱动，数据说话。永远不要凭感觉做优化，每次改动前后都要有数据对比。过早优化是万恶之源，但必要的优化不可拖延。"""
    },
    {
        ...
        "sort_order": 8, "knowledge_point": "全链路压测", "time_estimate": 35,
...
LESSON_CONTENT_4["安全测试基础"] = [
    {
        ...
        "sort_order": 1, "knowledge_point": "Web安全概述", "time_estimate": 30,
...
> **小结**：安全测试工程师的价值在于，能像攻击者一样思考，像防御者一样行动。"""
    },
    {
        ...
        "sort_order": 2, "knowledge_point": "SQL注入", "time_estimate": 35,
...
> **小结**：防御SQL注入非常简单——永远使用参数化查询，永远不要拼接SQL字符串。做到这一点，90%以上的SQL注入风险即可杜绝。"""
    },
    {
        ...
        "sort_order": 3, "knowledge_point": "XSS攻击", "time_estimate": 35,
        "content": """## XSS跨站脚本攻击详解
...
> **小结**：XSS防御口诀——输入验证、输出编码、CSP兜底、Cookie加固。"""
    },
    {
        ...
        "sort_order": 4, "knowledge_point": "CSRF/SSRF", "time_estimate": 35,
...
> **小结**：CSRF防御重在"谁在发请求"（Token/SameSite），SSRF防御重在"请求发到了哪里"（白名单/内网过滤）。"""
    ...
    {
        "title": "第5节：文件上传漏洞与防御",
        "sort_order": 5, "knowledge_point": "文件上传漏洞", "time_estimate": 30,
        "content": """## 文件上传漏洞与防御
...
> **小结**：核心原则——将上传目录与代码执行环境完全隔离，即使上传了恶意脚本也无法执行。"""
    },
    {
        ...
        "sort_order": 6, "knowledge_point": "认证授权安全", "time_estimate": 35,
        "content": """## 认证与授权安全
...
> **小结**：短时效Access Token+长时效Refresh Token+Token撤销机制=安全的JWT方案。"""
    },
    {
        ...
        "sort_order": 7, "knowledge_point": "Burp Suite工具", "time_estimate": 35,
...
> **小结**：Burp Suite是安全测试的"瑞士军刀"。掌握Proxy+Repeater+Intruder三大核心模块，足以应对大多数Web安全测试场景。工具是辅助，真正的能力来自于对漏洞原理的深刻理解。"""
    },
    {
        ...
        "sort_order": 8, "knowledge_point": "安全编码与DevSecOps", "time_estimate": 35,
...
LESSON_CONTENT_4["持续集成与DevOps"] = [
    {
        ...
        "sort_order": 1, "knowledge_point": "CI/CD概念", "time_estimate": 30,
...
> **小结**：CI/CD是现代软件工程的基石。核心要素：自动化、快速反馈、质量门禁、环境一致性。"""
    },
    {
        ...
        "sort_order": 2, "knowledge_point": "Git工作流", "time_estimate": 30,
        "content": """## Git工作流与分支策略
...
> **小结**：GitFlow适合有固定发布周期的传统团队，Trunk-Based适合追求快速迭代的互联网团队。核心原则：选择适合团队现状的模型，并坚持执行。"""
    },
    {
        ...
        "sort_order": 3, "knowledge_point": "Jenkins Pipeline", "time_estimate": 35,
...
> **小结**：声明式Pipeline适合大多数场景，脚本式Pipeline适合高度定制场景。Pipeline as Code是Jenkins使用的最佳实践，将构建流程版本化、可审查、可复用。"""
    },
    {
        ...
        "sort_order": 4, "knowledge_point": "Docker容器化", "time_estimate": 35,
...
> **小结**：Docker让环境一致性成为可能，"在我的机器上能跑"不再是问题。测试环境一键启动是Docker在测试中最直接的价值体现。"""
    },
    {
        ...
        "sort_order": 5, "knowledge_point": "GitLab CI/CD", "time_estimate": 35,
...
> **小结**：GitLab CI/CD的优势在于代码与CI配置一体化管理。核心设计理念：一切皆代码、Pipeline可视化、环境管理内置。"""
    },
    {
        ...
        "sort_order": 6, "knowledge_point": "质量门禁", "time_estimate": 30,
        "content": """## 质量门禁
