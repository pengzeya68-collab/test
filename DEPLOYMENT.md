...
### 坑15: 桌面端连远程服务显示"离线"，但服务明明在跑

**症状**: TestMaster 桌面版填入远程服务器地址后，登录页一直显示"离线 / 无法连接"，点"重新检测"也没用；但用浏览器访问该服务器的 Web 端一切正常，后端日志里几乎看不到桌面端的请求。

**原因**: 桌面端基于 `file://` 加载页面，浏览器发出的 `Origin` 头是 `null`。如果远程服务器的 `CORS_ORIGINS` 里没有 `null`，所有跨域请求（包括登录页的健康检查）都会被 CORS 静默拦截——请求根本到不了业务逻辑，所以后端日志里没有明显报错，极易误判为"服务没启动"。

**修复**:
```bash
# 在服务器的 .env 中，给 CORS_ORIGINS 追加 null
...
**教训**:
- 只要允许桌面端连接，`CORS_ORIGINS` 就必须包含 `null`（本机自带的后端已默认带上）
- 纯 Web 部署不需要桌面端时，可以去掉 `null` 收紧安全策略
- 排查"桌面端离线"时，第一步先看浏览器/网络层是否被 CORS 拦截，而不是反复重启服务
